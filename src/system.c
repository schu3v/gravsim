...
static void cleanup_sys(System *sys){
	free(sys->bodies_ren);
	free(sys->bodies);
}

static void realloc_sys(System *sys, char isren){
...
static int realloc_sys_or_cleanup_on_err(System *sys, char isren){
	Body *old_bodies = sys->bodies;
	Body_Ren *old_bodies_ren = sys->bodies_ren;
	
	realloc_sys(sys, isren);
...
int read_system(System *sys, char *filepath, char isren){
	...
	if (!realloc_sys_or_cleanup_on_err(sys, isren)){
		fclose(file);
		return 0;
	}
